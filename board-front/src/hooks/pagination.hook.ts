...
const usePagination = <T>(viewCount: number, sectionCount: number) => {
    ...
    // ! 객체)
    // Description: 객체 타입은 댓글, 게시물 등 다양한 타입이 올 수 있음 -> Generic 이용
    //                  State : 전체 객체 리스트 상태                   //
    ...
    const getViewList = () => {
        ...
        const tmpViewList = totalList.slice(firstIndex, (totalList.length > lastIndex ? lastIndex : totalList.length));
    ...
    const getViewPageList = () => {
        ...
        const tmpViewPageList = totalPageList.slice(firstIndex, (totalPageList.length > lastIndex ? lastIndex : totalPageList.length));
